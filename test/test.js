...
Test 8: Fifth order, homogeneous.
...
function runCorrectnessTests( memory_opt = false
  ...
  // Notes: 
  //   1. The expected outputs for these tests have been verified with WolframAlpha.
  //   2. Larger inputs are tested first, to ensure that the functions are working independent of persistent memory.
  let test_inputs = sampleDomain(0,50,1),
      // Test 1
      ...
      e2 = test_inputs.map(n => round(((5+sqrt(5))/10)*((1+sqrt(5))/2)**n + ((5-sqrt(5))/10)*((1-sqrt(5))/2)**n)).filter(size_filter),
      // Test 3
      t3 = test_inputs.map(solveLRR([1,-1], [-1,1], (_ => 0), memory_opt)).filter(size_filter),
      e3 = test_inputs.map(n => round(sqrt(3)*Math.sin(Math.PI*n/3) - Math.cos(Math.PI*n/3))).filter(size_filter),
      // Test 4
      t4 = test_inputs.map(solveLRR([2], [4], (n => 3**n), memory_opt)).filter(size_filter),
      e4 = test_inputs.map(n => 2**n + 3**(n+1)).filter(size_filter),
      // Test 5
      t5 = test_inputs.map(solveLRR([2,-1], [7,19], (n => 2**n + 2), memory_opt)).filter(size_filter),
      e5 = test_inputs.map(n => n*(n+7) + 2**(n+2) + 3).filter(size_filter),
      // Warning: the remaining tests are not as scalable.
      // They depend on explicit values rather than test_inputs and closed forms.
      // Test 6
      t6 = [9,8,7,6,5,4,3,2,1,0].map(solveLRR([-1,3,-2], [0,3,-1], (_ => 0), memory_opt)),
      e6 = [2018, -805, 319, -128, 51, -19, 10, -1, 3, 0],
      // Test 7
      t7 = [9,8,7,6,5,4,3,2,1,0].map(solveLRR([5,0,-7,2], [2,-1,0,3], (_ => 0), memory_opt)),
      e7 = [64678, 13751, 2919, 619, 128, 26, 3, 0, -1, 2],
      // Test 8
      t8 = [9,8,7,6,5,4,3,2,1,0].map(solveLRR([2,-3,4,-5,6], [1,-2,3,-4,5], (_ => 0), memory_opt)),
      e8 = [-10, 9, -8, 42, 50, 5, -4, 3, -2, 1],
      // All tests
      Tests = [t1,t2,t3,t4,t5,t6,t7,t8], Expected=[e1,e2,e3,e4,e5,e6,e7,e8],
      failed_tests = [];

  for(var i = 0; i < Tests.length; ++i)
  {
    if(!listElementsAreEqual(Tests[i], Expected[i]))
    {
      console.log("Correctness Test #" + (i+1).toString() + " Failed!")
      console.log("\tExpected Values: " + Expected[i].reverse());
      console.log("\tActual Values: " + Tests[i].reverse());
      failed_tests.push(i)
    }
  }
...
function runExecutionTimeTest()
{
  ...
  for(var i = 0; i < 100; ++i)
  {
    let f = solveLRR([1,-1], [-1,1], (_ => 0), true), // Test with memory optimization
        g = solveLRR([1,-1], [-1,1], (_ => 0), false), // Test without memory optimization.
        x = 100000*i;
...
let exe_times = runExecutionTimeTest()
